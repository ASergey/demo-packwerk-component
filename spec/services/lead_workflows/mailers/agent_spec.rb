...
RSpec.describe LeadWorkflows::Mailers::Agent do
  subject(:agent_mailer) { service.(**options) }
  ...
  let(:factory) { class_double(LeadWorkflows::EmailData::AgentEmail, new: factory_instance) }
  let(:factory_instance) { instance_double(LeadWorkflows::EmailData::AgentEmail, create: email_data) }
  ...
  it 'sends email' do
    ...
    expect(sendgrid).to have_received(:perform_async).with(email_data.id)
    expect(activity).not_to have_received(:perform_async)
